/*
  ...
  Step 2: Implement `initialize`, `advance`, `save`,
          and `load` in `pebble_chain`.
...
/* 4. (Optional:) Do you have any comments or suggestions
...
function pebble_chain() {
    ...
    chain.initialize = function(num_iterations, seed) {
        ...
        if (num_iterations != pow2 (num_pebbles)) {
            chain.state.stack.push ({index : 1,
                                     hash : hash (seed)});
        }

        var hash_value = hash (seed);
        for (var i = 1; i <= num_iterations; i++) {
            if (i == (num_iterations + 1 - pow2 (num_pebbles))) {
                chain.state.stack.push ({index : i,
                                         hash : hash_value});
                num_pebbles -= 1;
            }
    ...
    chain.advance = function() {
        ...
        if (chain.state.index == 0) return null;
        if (top (chain.state.stack).index != chain.state.index) {
            for (var i = top (chain.state.stack).index;
                 i < chain.state.index;
                 i++)
            {
                chain.state.stack.push (
                    {
                        index: i + 1,
                        hash : hash (top (chain.state.stack).hash)
                    }
                );
            }
        }
        chain.state.index -= 1;
        return chain.state.stack.pop().hash;
    };
